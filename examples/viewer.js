...
async function startViewer(localView, remoteView, formValues, onStatsReport, remoteMessage) {
    try {
        ...
        if (formValues.sessionId && formValues.sessionId.trim() !== '') {
            ...
            // Check for available sessions with CHANNEL_CREATED status
            try {
                const availableSessions = await getStreamingSessions(viewer.authToken);
                console.log('[VIEWER ADAPTER] Available sessions:', availableSessions);

                if (availableSessions && availableSessions.length > 0) {
                    // Pick the first available session
                    viewer.sessionData = availableSessions[0];
                    console.log('[VIEWER ADAPTER] Using available session:', viewer.sessionData.id);
                } else {
                    // Create a new streaming session if no available sessions
                ...
                console.error('[VIEWER ADAPTER] Error checking for available sessions:', error);
        ...
        viewer.clientId = formValues.clientId || Math.random().toString(36).substring(2, 15);
        ...
        if (!formValues.natTraversalDisabled && !formValues.forceSTUN) {
            viewer.channelData.ice_servers.forEach((iceServer) =>
        ...
        console.log('[VIEWER] ICE servers:', viewer.iceServers);
        // Find WSS endpoint
        viewer.wssEndpoint = viewer.channelData.endpoints.find((endpoint) => endpoint.Protocol === 'WSS')?.ResourceEndpoint;

        if (!viewer.wssEndpoint) {
        ...
        viewer.signalingClient = new KVSWebRTC.SignalingClient({
            ...
            channelEndpoint: viewer.wssEndpoint,
            clientId: viewer.clientId,
            role: 'VIEWER',
            region: viewer.channelData.region,
            systemClockOffset: 0,
            ...
            requestSigner: {
                getSignedURL: async () => {
                    ...
                    const paramsObj = viewer.channelData.signed_signaling_params;
                    ...
                    return `${viewer.wssEndpoint}?${params.toString()}`;
                },
            },
        });
